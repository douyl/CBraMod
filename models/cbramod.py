...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x, mask=None):    
        ...
        patch_emb = self.proj_in(mask_x)   # (B, 1, C*N, T) -> (B, 25, C*N, T/25)
        patch_emb = patch_emb.permute(0, 2, 1, 3).contiguous().view(bz, ch_num, patch_num, patch_size)  # (B, 25, C*N, T/25) -> (B, C*N, 25, T/25) -> (B, C, N, T)
        patch_emb = self.time_proj(patch_emb)  # (B, C, N, T) -> (B, C, N, D)

        mask_x = mask_x.contiguous().view(bz*ch_num*patch_num, patch_size)  # (B, 1, C*N, T) -> (B*C*N, T)
        spectral = torch.fft.rfft(mask_x, dim=-1, norm='forward')  # (B*C*N, T) -> (B*C*N, T//2+1)
        spectral = torch.abs(spectral).contiguous().view(bz, ch_num, patch_num, patch_size//2+1) # (B*C*N, T//2+1) -> (B, C, N, T//2+1)
        spectral_emb = self.spectral_proj(spectral)  # (B, C, N, T//2+1) -> (B, C, N, D)

        patch_emb = patch_emb + spectral_emb   # (B, C, N, D)

        positional_embedding = self.positional_encoding(patch_emb.permute(0, 3, 1, 2))  # (B, C, N, D) -> (B, D, C, N) -> (B, D, C, N)
        positional_embedding = positional_embedding.permute(0, 2, 3, 1)  # (B, D, C, N) -> (B, C, N, D)

        patch_emb = patch_emb + positional_embedding # (B, C, N, D)
